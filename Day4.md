...
// 这里说明一下，每次返回的oproxy都是同一个地址空间
// 若是下面这样
// var a = pipe(3);
// var b = pipe(3).double;
// console.log(a === b) // false
// 因为每次执行pipe(3)都会生成不同地址的Proxy实例对象
pipe(3).double.pow.reverseInt.get; // 63
```

下面的例子则是利用`get`拦截，实现一个生成各种 DOM 节点的通用函数`dom`。（给的有点陡，细品，最好学一下调试技巧）
...
// 若为target正常的对象（writable: true,configurable: true）那么这里应该返回abc
// 因为通过代理实例取访问一个属性都会调用get方法
proxy.foo
...
// 待我跟你絮叨絮叨
// 1. Reflect.apply()类似于Function.prototype.apply()
// 2. arguments部署了Iterator接口，所以可以使用扩展运算符进行赋值
// 3. 对于apply,call,bind这些能够改变函数执行上下文的调用方法，当传入的上下文（this）为undefined或null，那么就是全局对象，reflect.apply方法一样
```
...
```js
// 1. 对于没有设置handler对象的proxy实例就等于target对象，
// 2. 若handler对象中没有设置set方法和get方法，那么当你取值和赋值的时候就是直接对target对象进行的操作
...
`defineProperty`方法拦截了`Object.defineProperty`操作。（书中给的例子不好，用我这个）
...
var target = {};
var proxy = new Proxy(target, handler);
// 会调用handler的defineProperty方法，同时返回proxy实例
var heihi = Object.defineProperty(proxy,'fuck',{
    value: 'you',
    writable: true,
    configurable: true,
    enumerable: true
...
console.log(heihi);
proxy.foo = 'bar' // 只调用set方法
console.log(heihi.fuck); // you
console.log(proxy.fuck); // you
console.log(proxy.foo); // bar

// 注意
// 1. 对于同时存在set和defineProperty，若进行赋值操作只会调用set
// 2. 对于只存在defineProperty方法，那么进行赋值操作会调用这个方法，因为当你新建一个属性的时候，会默认给他定义属性描述符且都为true
```
...
var handler = {
  ...
  Object.preventExtensions(target);
  var proxy = new Proxy(target, handler);
  var fuck = Object.setPrototypeOf(proxy, proto);
  console.log(fuck);
```

运行结果

![image-20200503100255728](./image-20200503100255728.png)
...
同理，Proxy 也可以用来实现数据库的 ORM 层(很多框架对数据库都做了orm层抽象，方便操作数据库)。
...
// 2. 方法二
console.log(Math.floor.apply(undefined,[1,75]));  // 1
...
据对象`person`是观察目标，函数`print`是观察者。一旦数据对象发生变化，`print`就会自动执行。
